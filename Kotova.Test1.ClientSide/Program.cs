...
namespace Kotova.Test1.ClientSide
{
    internal static class Program
    {
        ...
        { 
            ...
            {
                MessageBox.Show("Îáíîâëåíèå ïðîãðàììû â ïðîöåññå. Ïîæàëóéñòà, äîæäèòåñü îêîí÷àíèÿ îáíîâëåíèÿ.");
                return;
            }
            ...
            if (environment != "Development")
            {
                ...
                if (!string.Equals(AppDomain.CurrentDomain.BaseDirectory.TrimEnd(Path.DirectorySeparatorChar), targetDirectory, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    CreateNewShortcutForAppOnTheDesktop(targetPath); //TODO: MAIN PRIORITY. ÏÐÎÂÅÐÈÒÜ, ×ÒÎ ÝÒÀ ØÒÓÊÀ ÐÀÁÎÒÀÅÒ!
                ...
                if (!string.IsNullOrEmpty(embeddedVersionInfo.ServerInternalFilePath))
                {
                    ...
                    else
                    {
                        if (IsNewerVersion(embeddedVersionInfoFromServer.Version, embeddedVersionInfo.Version))
                        {
                            ...
                            MessageBox.Show(
                                "Äîñòóïíà íîâàÿ âåðñèÿ ïðèëîæåíèÿ. Ñêà÷èâàåì è îáíîâëÿåì.",
                                "Îáíîâëåíèå"
                            );
            ...
            else
            {
                if (environment == "Development")
                {
                    MessageBox.Show("Íå óäàëîñü çàãðóçèòü âåðñèþ èç ôàéëà .json ñ ñåðâåðà äëÿ îáíîâëåíèÿ ïðîãðàììû.");
                }
            }
        ...
        private static bool IsNewerVersion(string remoteVersion, string localVersion)
        {
            ...
            {
                MessageBox.Show("Óäàë¸ííàÿ âåðñèÿ ôàéëà ñåðâåðà íå íàéäåíà. (Ñêîðåå âñåãî íåò ïîäêëþ÷åíèÿ ê óäàë¸ííîìó ñåðâåðó)");
                return false;
            }
        ...
        private static string? UpdateApplication(string fileUrl, string version)
        {
            ...
            try
            {
                progressForm.SetMessage("Ñêà÷èâàåòñÿ îáíîâëåíèå. Ïîæàëóéñòà, ïîäîæäèòå...");
                progressForm.Show();
                ...
                exeClient.DownloadFileCompleted += (s, e) =>
                {
                    if (e.Error != null)
                    {
                        MessageBox.Show($"Íå óäàëîñü ñêà÷àòü EXE-ôàéë: {e.Error.Message}");
                        progressForm.Close();
                        RemoveUpdateLock();
                        return;
                    }
                    ...
                    progressForm.SetMessage("EXE-ôàéë çàãðóæåí. Ñêà÷èâàåòñÿ appsettings.json...");
                    ...
                    jsonClient.DownloadFileCompleted += (snd, evt) =>
                    {
                        if (evt.Error != null)
                        {
                            MessageBox.Show($"Íå óäàëîñü ñêà÷àòü appsettings.json: {evt.Error.Message}");
                            progressForm.Close();
                            RemoveUpdateLock();
                            return;
                        }
                        ...
                        progressForm.SetMessage("Îáíîâëåíèå ñêà÷åíî è óñòàíîâëåíî. Ïåðåçàïóñê...");
            ...
            {
                MessageBox.Show($"Íå óäàëîñü îáíîâèòü ïðèëîæåíèå: {ex.Message}");
                progressForm.Close();
                RemoveUpdateLock();
                return null;
            }
